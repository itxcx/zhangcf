...
// +----------------------------------------------------------------------
// $Id: UploadFile.class.php 774 2010-10-28 06:47:02Z yuetao $
 ...
 * @subpackage  Net
 * @author    liu21st <liu21st@gmail.com>
 * @version   $Id: UploadFile.class.php 774 2010-10-28 06:47:02Z yuetao $
 +------------------------------------------------------------------------------
 */
...
{//类定义开始
    ...
    private function save($file)
    {
		...
		//非法文件名判断,文件名称不可包含2个及以上的., 其他符号也不能包含如 ;
    ...
    public function upload($savePath ='')
    {
        ...
        foreach($files as $key => $file) {
            ...
            if(!empty($file['name'])) {
				...
				$file['file']		= $savePath.$file['savename'];		//文件路径
				$file['thumbfile']	= $file['savepath'].$this->thumbPrefix.$file['savename'].$this->thumbSuffix; //缩略图路径
                ...
                if(function_exists($this->hashType)) {
                    $fun =  $this->hashType;
					$file['hash']   =  $this->thumb?$fun(mb_convert_encoding($file['savepath'].$this->thumbPrefix.$file['savename'].$this->thumbSuffix,'utf-8','gbk')):$fun(mb_convert_encoding($file['savepath'].$file['savename'],'utf-8','gbk'));
                }
                //上传成功后保存文件信息，供其他地方调用
    ...
    {
		$ftime  = explode('.',microtime(true));
		$itime	= $ftime[0].$ftime[1];
        $rule	= $itime;
        if(empty($rule)) {//使用给定的文件名作为标识号
			 $saveName = $rule.rand(1,999).".".$filename['extension'];
        }else {
