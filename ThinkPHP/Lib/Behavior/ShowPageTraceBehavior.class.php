...
class ShowPageTraceBehavior extends Behavior {
    ...
    private function showTrace1() {
        ...
        foreach ($files as $file){
            ...
            if(!preg_match('/ThinkPHP/',$file) && !preg_match('/runtime/',strtolower($file))){
                if(!($_SERVER['SERVER_ADDR']=="127.0.0.1" || $_SERVER['REMOTE_HOST']=="127.0.0.1"))
                    $file=substr($file,strlen($_SERVER['DOCUMENT_ROOT'])+1,strlen($file));
                $n=explode('\\',$file);
                $name=$n[count($n)-1];
               if(preg_match('/action/i',$file)){
                    $xsdebug.="<afile><file path='$file' name='$name' /></afile>";
                }else if(preg_match('/model/i',$file)){
                    $xsdebug.="<mfile><file path='$file' name='$name' /></mfile>";
                }else{
                    $xsdebug.="<ofile><file path='$file' name='$name' /></ofile>";
                }
            }
            
                
        }
        $xsdebug=str_replace('\\','/',$xsdebug);  
        $self=md5(trim(__SELF__).'a');
        $_SESSION[$self]=$xsdebug;
    }
