...
exit;
    ...
    public function public_dispatchJump()
    {
    	$status =$_POST['status'];
    	...
    	if(isset($_POST['waitSecond']))
    	{
    		$this->assign('waitSecond',$_POST['waitSecond']);
    	}
        // 提示标题
        $this->assign('msgTitle',$status? L('_OPERATION_SUCCESS_') : L('_OPERATION_FAIL_'));
        //如果设置了关闭窗口，则提示完毕后自动关闭窗口
        if(isset($_POST['closeWin']))    $this->assign('jumpUrl','javascript:window.close();');
        ...
        if($status==1) { //发送成功信息
            ...
            // 默认发生错误的话自动返回上页
            if(!$this->view->get('jumpUrl')) $this->assign('jumpUrl',"javascript:history.back(-2);");
    ...
    private function dispatchJump($message,$status=1,$jumpUrl='',$ajax=false) {
		...
		if($this->view->get('newJump'))
        {
			echo '<html>';
			echo '<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /></head>';
			echo '<body>';
			echo "<form id='dispatchJump' name='dispatchJump' action='".__URL__."/public_dispatchJump' method='post'>";
			echo "<input type='hidden' name='status' value='".$status."'/>";
			echo "<input type='hidden' name='message' value='".$message."'/>";
			if($this->view->get('closeWin'))
			{
				echo "<input type='hidden' name='closeWin' value='1'/>";
			}
			if(!empty($_SERVER["HTTP_REFERER"])){
				echo "<input type='hidden' name='http_referer' value='".$_SERVER["HTTP_REFERER"]."'/>";
			}
			if($this->view->get('jumpUrl'))
			{
					echo "<input type='hidden' name='jumpUrl' value='".$this->view->get('jumpUrl')."'/>";
			}
			if($this->view->get('waitSecond'))
			{
					echo "<input type='hidden' name='waitSecond' value='".$this->view->get('waitSecond')."'/>";
			}
			echo "</form>";
			echo "<script>document.forms['dispatchJump'].submit();</script>";
			echo '</body>';
			echo '</html>';
			tag('app_end');
			exit;
