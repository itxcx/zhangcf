...
class ZipStream {
	...
	function __construct($archiveName = "") {
		$this->fstream = fopen($archiveName,"a");
	}
	...
	public function openStream($filePath, $timestamp = 0, $fileComment = null)   {
		...
		$this->streamFile = tempnam(ROOT_PATH, 'ZipStream');
	...
	public function addStreamData($data) {
		...
		if ($length != strlen($data)) {
			...
			throw_exception("Length mismatch!");
		}
	...
	public function closeStream() {
		...
		while(!feof($file_handle)) {
			fwrite($this->fstream, fread($file_handle, $this->streamChunkSize));//print fread($file_handle, $this->streamChunkSize);
		}
		//flush();
	...
	public function finalize() {
		if(!$this->isFinalized) {
			...
			$gzstr .= $cd;								//print($cd);
			$gzstr .= $this->endOfCentralDirectory;		//print($this->endOfCentralDirectory);
			$gzstr .= pack("v", sizeof($this->cdRec));	//print(pack("v", sizeof($this->cdRec)));
			$gzstr .= pack("v", sizeof($this->cdRec));	//print(pack("v", sizeof($this->cdRec)));
			$gzstr .= pack("V", strlen($cd));			//print(pack("V", strlen($cd)));
			$gzstr .= pack("V", $this->offset);			//print(pack("V", $this->offset));
			if (!is_null($this->zipComment)) {
				$gzstr .= pack("v", strlen($this->zipComment));	//print(pack("v", strlen($this->zipComment)));
				$gzstr .= $this->zipComment;					//print($this->zipComment);
			} else {
				$gzstr .= "\x00\x00";							//print("\x00\x00");
			}
			
			
			fwrite($this->fstream, $gzstr);
