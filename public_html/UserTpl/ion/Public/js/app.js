	...
	$.jump_time = $.root_.find("#jump_time");
	var jump_time_out = null;
...
function _ln(txt)
{
//	var _ln_bool = GLOBAL._LN_BOOL;
//	return _ln_bool ? GLOBAL._LN[txt] : txt;
...
$(document).ready(function() {
	...
	$('button[data-jump]').on('click', function() {
		//console.log("ready");
		var btn = $(this)
		btn.button('loading');
		Change_ad_url();
		jump_time_out = setTimeout(Jump_time_change, 1000);
		$("#content").hide();
		$("#content_ad").removeClass("hide");
		//$("#notify_ad").modal();
		setTimeout(function() {
			btn.button('reset');
			//$("#notify_ad").modal('hide');
			$("#content_ad").addClass("hide");
			$("#content").show();
		}, 5000)
	});
	...
	$('#refresh').click(function(e) {
		...
		var $this = $(this);
		var ln = GLOBAL._LN_BOOL;
		//console.log(ln);
		var tis = ln ? '清除缓存' : 'Clear Local Storage';
		$.widresetMSG = $this.data('reset-msg');
		
		$.SmartMessageBox({
			title : "<i class='fa fa-refresh' style='color:green'></i> " + tis,
	...
	$('#logout a').click(function(e) {
		...
		var ln = GLOBAL._LN_BOOL;
		//console.log(ln);
		var tis = ln ? '登出' : 'Logout';
		$.loginURL = $this.attr('href');
		$.logoutMSG = $this.data('logout-msg');

		// ask verification
		$.SmartMessageBox({
			title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> "+ tis +" <span class='txt-color-orangeDark'><strong>" + $('#show-shortcut').text() + "</strong></span> ?",
...
function runAllCharts() {
	...
	if ($.fn.easyPieChart) {
		...
		$('.easy-pie-chart').each(function() {
			...
			var time = $this.data('pie-time') || 1500;
			$this.easyPieChart({
				barColor : barColor,
...
function checkURL() {
	...
	if (url) {
		...
		document.title = 'ion² - ' + (title || document.title);
...
function drawBreadCrumb() {
	...
	$.bread_crumb.empty();
	$.bread_crumb.append($("<li>"+ _ln("Home") +"</li>"));
	nav_elems.each(function() {
		...
		if (!--count) document.title = 'ion² - ' + $.bread_crumb.find("li:last-child").text();
	});
...
* change AD url
*/
function Change_ad_url()
{
	var url = 'img/jinfan/GRG-Ventures--banner-';
	var num = Number((Math.random() * 10 + 1) >> 0);
	var src = "";
	var img = new Image();
...
function Jump_time_change()
{
	//console.log("fff");
	//console.log("fun");
	var num = Number($.jump_time.text());
	if(0 < $.jump_time.text())
	{
		$.jump_time.text(--num);
		setTimeout(Jump_time_change, 1000);
	}
	else
	{
		jump_time_out =  null;
...
* 检查安全密码是否正确
*/
function Check_security_password(pwd, callback)
{
	if("" == pwd){ return false; }
	var result = false;
	$.post('database/Checkdb.php', {type : "security_pwd", pwd : pwd}, function(data)
	{
		if(data)
		{
			result = data["result"];
			if(result)
			{
				...
				{
					$.root_.find("#widget-grid").removeClass("hide");
				}
			...
			else
			{
				...
				setTimeout(function(){Write_security_password("error_pwd", callback)}, 500);
			}
		}
	},"json");
}
...
/*调用检验安全密码*/
function  Write_security_password(tis, callback)
{
	//console.log(GLOBAL._LN_BOOL);
	var title = 'error_pwd' != tis ? _ln('Please enter your security password') : _ln('Security password error, Please try again.') ;
	$.SmartMessageBox(
	{
		title : "<i class='fa fa-key txt-color-orangeDark'></i> "+ title,
		content : $.logoutMSG || _ln("You must enter your password to the next step"),
		buttons : '[No][Yes]',
		input : "password",
		inputValue : "",
		placeholder : _ln("Enter your security password")
		
	}, function(ButtonPressed, Value) 
	{
		if ("Yes" === ButtonPressed) 
		{
			Check_security_password(Value, callback);
		}
		else if ("No" === ButtonPressed) 
		{
			location.href = './';
		}
...
function Change_ln(txt)
{
	var _ln_bool = GLOBAL._LN_BOOL;
	var _ln = GLOBAL._LN;
	return _ln_bool ? _ln[txt] : txt;
}
...
* @tis tis text
*/
function Notify_tis(bool, title, tis, timeout)
{
	if("" == title)
	{
		return false;
	}
	var color = bool ? "#739E73" : "#C79121";
	var time = undefined === timeout ? 8000 : timeout
	$.smallBox({
		title : title,
		content : tis || "",
		color : color,
...
//数字转格式  e.g. 2000->2K  2000000->2M
function FromatAmount(num)
{
	//var obj = $(id);
	//var amount = (obj.text()).substring(1);
	...
	{
		result = Number((result / 1000).toFixed(1)) + "K";
	}
	else
	{
		result = Number((result / 1000000).toFixed(1)) + "M";
	}
	return result;
	//$(id).text(result);
}
