...
class Worker
{
    ...
    public static function init()
    {
        ...
        {
        	//生成主进程的文件名 命名规则 执行 执行的命令地址加上子进程的地址
            self::$_startFile = $argv[0].'/Applications/'.$argv[2].'/start.php';
    ...
    public static function parseCommand()
    {
        ...
        {
            exit("Usage: php yourfile.php {start|stop|restart|reload|status child}\n");
        }
        // 命令
        $command = trim($argv[1]);
        //运行的子文件
        $command2 = isset($argv[2]) ? $argv[2] : 'all';
        // 子命令，目前只支持-d
        $command3 = isset($argv[3]) ? $argv[3] : '';
        
        // 记录日志
        self::log("$command:$command2");
        ...
        switch($command)
        {
            ...
            // 停止 workeran
            case 'stop':
                self::log("Workerman[".$start_file." ./Applications/".$argv[2]."/start.php] is stoping ...");
                ...
                while(1)
                {
                    ...
                    if($master_is_alive)
                    {
                        ...
                        {
                            self::log("Workerman[".$start_file." ./Applications/".$argv[2]."/start.php] stop fail");
                            exit;
                        }
                        usleep(10000);
                        continue;
                    }
                    self::log("Workerman[".$start_file." ./Applications/".$argv[2]."/start.php] stop success");
                    // 是restart命令
                    if($command === 'stop')
                    {
                        exit(0);
            ...
            case 'reload':
                ...
                self::log("Workerman[".$start_file." ./Applications/".$argv[2]."/start.php] reload");
                exit;
            // 未知命令
            default :
                 exit("Usage: php yourfile.php {start|stop|restart|reload|status child}\n");
