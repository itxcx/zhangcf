...
class Verify {
    ...
    public function check($code, $id = '') {
        ...
        // session 过期
        if(systemTime() - $secode['verify_time'] > $this->expire) {
            session($key, null);
            return "验证码超时";
        }
    ...
    public function entry($id = '') {
        ...
        $secode['verify_time'] = systemTime();  // 验证码创建时间
    ...
    private function _writeNoise() {
        ...
        for($i = 0; $i < 10; $i++){
            ...
            for($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring($this->_image, 5, mt_rand(-15, $this->imageW),  mt_rand(-10, $this->imageH), $codeSet[mt_rand(0, 29)], $noiseColor);
            }
        }
    }
