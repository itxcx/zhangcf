...
class Page {
    ...
    protected $coolPages   ;
    // 分页显示定制
    //  protected $config  =	array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'共有%totalRow% %header% %nowPage%/%totalPage% 页 %upPage% %downPage% %first%  %prePage%  %linkPage%  %nextPage% %end%');
    protected $config  =	array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'首页','last'=>'尾页','theme'=>'共有%totalRow% %header% %nowPage%/%totalPage% 页 %first% &nbsp; %upPage% &nbsp;%linkPage% &nbsp;&nbsp; %downPage% &nbsp; %end%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
        ...
        if($this->nowPage == $this->coolPages){
            $nextPage = "";
