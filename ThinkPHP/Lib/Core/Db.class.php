...
class Db {
    ...
    protected function debug() {
        ...
        if(C('LOG_RUNSQL'))
        {
        	if($this->queryStr)
        	{
		        if(!is_dir(dirname(APP_PATH.'../Admin/Runtime/Logs/runsql.sql')))
		            // 如果静态目录不存在 则创建
		            mkdir(dirname(APP_PATH.'../Admin/Runtime/Logs/runsql.sql'),0777,true);
	        	file_put_contents(APP_PATH.'../Admin/Runtime/Logs/runsql.sql',$this->queryStr."\n",FILE_APPEND);
		        ...
		        if(strpos($this->queryStr,'SHOW COLUMNS FROM')===false)
		        {
			        foreach($trace as $key=>$trac)
			        {
			        	...
			        	)
			        	{
			        		if($trace[$key]['file']!='')
			        		{
				        		file_put_contents(APP_PATH.'../Admin/Runtime/Logs/runsql.sql','-- ['.Date('Y-m-d H:i:s').']'.iconv('GB2312', 'UTF-8', $trace[$key]['file']).'('.$trace[$key]['line'].')'."\n",FILE_APPEND);
			        		}
			        	}
			        }
		        }
